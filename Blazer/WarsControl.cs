...
public class WarsControl
{
    ...
    public Commands GetPlayerCommands(int playerIndex, ref WarsContent warsContent
#if HAVE_LEGACY_DIRECTX
        , ref Microsoft.DirectX.DirectInput.JoystickState joystickState
#endif
        )
    {
        ...
        */
        ...
        if (me.ShipType == Constants.SHIP_TYPE_MANUALLY_CONTROLLED)
        {
            ...
            if (joystickState.X == 65535)
            {
                commands.right = true;
            }
            if (buttons[4] >= 128 || buttons[5] >= 128 || buttons[6] >= 128 || buttons[7] >= 128)
            {
                commands.throttle = true;
            }
            if (buttons[0] >= 128 || buttons[1] >= 128 || buttons[2] >= 128 || buttons[3] >= 128)
            {
                commands.fire = 1;
            }
#endif
